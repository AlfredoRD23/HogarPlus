  ...
  return (
    <div className="panel overflow-hidden">
      <div className="flex items-center justify-between border-b px-5 py-4">
        ...
        {typeof count === "number" && (
          <span className="rounded-full bg-gold-100 px-2.5 py-1 text-xs font-medium text-navy-900">{count}</span>
        )}
      </div>
      {loading ? (
        ...
        <>
          ...
          <div className={mobile ? "hidden overflow-auto md:block" : "overflow-auto"}>
            <table className="data-table">
              <thead className="bg-navy-900 text-xs font-medium text-gold-300">
                <tr>
                  {headers.map((h) => (
                    <th key={h}>
                      {h}
