...
import { FormattedInput, Modal } from "../components/Form";
    ...
    <div className="min-h-screen bg-navy-950 text-white">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-16 sm:px-6">
        ...
        <p className="mt-2 max-w-2xl text-slate-300">
          Consulta tus productos, cuotas pendientes y pide algo nuevo de tu categoría.
        </p>
        <form
            ...
            <section className="rounded-3xl bg-white p-5 text-navy-900 sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {data.catalog.map((product) => (
                  <div
                    key={product.id}
                    className={`flex flex-col rounded-2xl border p-4 ${product.canRequest ? "border-slate-200 bg-white" : "border-slate-100 bg-slate-50"}`}
                  >
...
function CreditCard({ credit }: { credit: PortalCredit }) {
